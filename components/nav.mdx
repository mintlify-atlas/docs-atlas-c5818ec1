...
```jsx
import logowhite from "@assets/ok.svg";          // shown in light mode
import logoblack from "@assets/ok_white_bgsvg.svg"; // shown in dark mode
...
<Note>
  `ok.svg` uses a white foreground (suitable for dark backgrounds). `ok_white_bgsvg.svg` uses a dark foreground (suitable for light backgrounds). The names can seem counter-intuitive — always rely on the `isDarkMode` boolean rather than the file name.
</Note>
